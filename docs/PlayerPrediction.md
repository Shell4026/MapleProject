...
핑 1000ms 기준 teleport기반 방식은 반응이 느려지는 것을 볼 수 있습니다.(모션은 나오지만 이동을 못함)</br>
...
> [!NOTE]
> 클라이언트로부터 받은 입력은 즉시 처리하는 것이 아니라, 클라와 서버의 틱 차이를 계산하고 서버는 그 차이만큼 늦게 처리합니다.
> 그렇게 하지 않으면 아주 짧은 입력의 경우 키를 뗄 때까지 클라이언트는 3틱만큼 물리 연산을 했지만, 서버는 5틱만큼 처리하는 경우가 생겨 예측에 오차가 발생합니다.
