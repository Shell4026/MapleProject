...
## 핵심 특징
- 멀티 월드 서버 시뮬레이션
- TCP/UDP 기반 통신 [상세](docs/NetworkAndSync.md)
- EventBus + Router 기반 패킷 라우팅(낮은 결합도) [상세](docs/Architecture.md#패킷-라우팅-설계)
- 플레이어 입력 예측/재조정(Prediction/Reconciliation) [상세](docs/PlayerPrediction.md)
- 확장이 쉬운 데이터 기반 스킬 시스템 [상세](docs/SkillSystem.md)
- FSM 기반 몹 AI, 스킬/히트박스 처리, 아이템 드롭/획득
- 인벤토리 변경 슬롯 기반 동기화 + DB 반영 [상세](docs/DataPersistence.md)
